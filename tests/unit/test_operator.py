...
class TestCharm:
    ...
    def test_validate_default_s3_bucket_failure_wrong_name(
        ...
        relational_db_relation.begin_with_initial_hooks()
        assert relational_db_relation.charm.model.unit.status == BlockedStatus(
            "Invalid value for config default_artifact_root 'mlflow' - value must be a valid S3 bucket name"
        )
    ...
    def test_validate_default_s3_bucket_failure_bucket_creation(
        ...
        relational_db_relation.begin_with_initial_hooks()
        assert relational_db_relation.charm.model.unit.status == BlockedStatus(
            "Error with default S3 artifact store - bucket not accessible or cannot be created.  Caught error: '"
        )
    ...
    def test_validate_default_s3_bucket_failure_bucket_creation_not_allowed(
        ...
        relational_db_relation.begin_with_initial_hooks()
        assert relational_db_relation.charm.model.unit.status == BlockedStatus(
            "Error with default S3 artifact store - bucket not accessible or does not exist. Set create_default_artifact_root_if_missing=True to automatically create a missing default bucket"
        )
